...
use windows_sys::{
    ...
    Win32::{
        ...
        System::Threading::{
            CreateProcessW, WaitForSingleObject, CREATE_UNICODE_ENVIRONMENT, INFINITE,
...
pub fn create_child_process_with_pwd_per_drive(
    ...
    for (key, value) in env_vars {
        let mut entry: Vec<u16> = OsString::from(format!("{}={}", key, value))
            .encode_wide()
            .collect();
    ...
    let application_wide: Vec<u16> = OsString::from(application)
        .encode_wide()
        .chain(Some(0))
        .collect();
    let cmdline_wide: Vec<u16> = OsString::from(cmdline)
        .encode_wide()
        .chain(Some(0))
        .collect();
